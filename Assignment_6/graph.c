...
int dfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
  ...
  while(!dequeIsEmpty(stack)) {
    ...
    for(int i = 0; i < currentNode->numNeighbors; i++) {
      ...
      if(!(currentNode->neighbors[i]->isVisited)) {
	dequePushBack(stack, currentNode->neighbors[i]);
...
int bfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
  ...
  // FIXME: Implement
	...
	dequePopFront(queue);
        ...
        for(int i = 0; i < currentNode->numNeighbors; i++) {
	  if(currentNode->neighbors[i]->isVisited == 0) {
            dequePushBack(queue, currentNode->neighbors[i]);
            currentNode->neighbors[i]->isVisited = 1;
	  }
        }
